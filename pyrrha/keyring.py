...
# Adapted from Pithos' util.py SecretService (C) 2010-2012 Kevin Mehall.
#
...
This mirrors Pithos' ``_SecretService`` but depends only on libsecret (no
GTK), so importing it does not pull GTK into the Qt process.  Pyrrha stores the
password under its own ``io.github.k5blazerfl.Pyrrha.Account`` schema (a
one-time migration copies an existing Pithos password across).  The async
libsecret calls dispatch through the GLib main context, which the
:class:`pyrrha.glib_bridge.GLibBridge` keeps serviced.
"""
...
class _SecretService:
    ...
    _account_schema = Secret.Schema.new(
        'io.github.k5blazerfl.Pyrrha.Account',
    ...
    def unlock_keyring(self, callback):
        ...
        Secret.Service.get(Secret.ServiceFlags.NONE, None, on_get_finish, None)
    ...
    def get_account_password(self, email, callback):
        ...
        if is_flatpak():
            try:
                password = Secret.password_lookup_sync(
                    self._account_schema, {'email': email}, None,
                ) or ''
                callback(password)
            except GLib.Error as e:
        ...
        Secret.password_lookup(
            self._account_schema, {'email': email}, None, on_password_lookup_finish,
        )
        ...
        def on_password_clear_finish(source, result, data):
            try:
                ...
                Secret.password_store(
                    self._account_schema, {'email': new_email}, self._current_collection,
                    'Pandora Account', password, None, on_password_store_finish, None,
                )

        if old_email and old_email != new_email:
            Secret.password_clear(
                self._account_schema, {'email': old_email}, None,
                on_password_clear_finish, None,
            )
        else:
            Secret.password_store(
                self._account_schema, {'email': new_email}, self._current_collection,
